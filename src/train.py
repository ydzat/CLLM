...
def main() -> None:
    ...
    if args.resume:
        ...
        # The checkpoint carries the optimizer's OLD (decayed) LR; schedulers read
        # param_group["initial_lr"], and load_state_dict restores it stale, so reset
        # BOTH lr and initial_lr to the config LR before building the WSD schedule.
        for group in opt.param_groups:
            group["lr"] = tcfg["lr"]
            group["initial_lr"] = tcfg["lr"]
